...
-/
import NRR.OddSphereDegree.AlgebraicTopology.Backports.SimplicialObjectHomotopy
...
Only the module-system syntax and the `using!` simpa variant (replaced by
`using`) were adapted; the mathematical content is upstream verbatim.
...
/-
Pure combinatorial disjointness fact #1 used in `comm_succ`.
-/
private lemma disj1 (n : ℕ) :
    Disjoint (Finset.image (fun x : Fin (n + 1) × Fin (n + 2) => (x.2.castSucc, x.1.succ))
        (({x : Fin (n + 1) × Fin (n + 2) | x.1.castSucc < x.2} : Finset _)ᶜ))
      (Finset.image (fun x : Fin (n + 1) × Fin (n + 2) => (x.2.succ, x.1.castSucc))
        ({x : Fin (n + 1) × Fin (n + 2) | x.1.castSucc < x.2} : Finset _)) := by
  simp +decide [ Finset.disjoint_right ];
  grind +suggestions

/-
Pure combinatorial disjointness fact #2 used in `comm_succ`.
-/
private lemma disj2 (n : ℕ) :
    Disjoint (Finset.image (fun i : Fin (n + 1) => (i.castSucc.succ, i.succ)) Finset.univ)
      (Finset.image (fun i : Fin (n + 1) => (i.castSucc.succ, i.castSucc)) Finset.univ) := by
  simp +decide [ Finset.disjoint_right ];
  grind

/-
Pure combinatorial disjointness fact #3 used in `comm_succ`.
-/
private lemma disj3 (n : ℕ) :
    Disjoint ((Finset.image (fun i : Fin (n + 1) => (i.castSucc.succ, i.succ)) Finset.univ) ∪
        (Finset.image (fun i : Fin (n + 1) => (i.castSucc.succ, i.castSucc)) Finset.univ))
      ({(0, 0), (Fin.last (n + 2), Fin.last (n + 1))} :
        Finset (Fin (n + 2 + 1) × Fin (n + 1 + 1))) := by
  simp +zetaDelta at *

/-
Pure combinatorial disjointness fact #4 used in `comm_succ`.
-/
private lemma disj4 (n : ℕ) :
    Disjoint ((Finset.image (fun x : Fin (n + 1) × Fin (n + 2) => (x.2.castSucc, x.1.succ))
          (({x : Fin (n + 1) × Fin (n + 2) | x.1.castSucc < x.2} : Finset _)ᶜ)) ∪
        (Finset.image (fun x : Fin (n + 1) × Fin (n + 2) => (x.2.succ, x.1.castSucc))
          ({x : Fin (n + 1) × Fin (n + 2) | x.1.castSucc < x.2} : Finset _)))
      (((Finset.image (fun i : Fin (n + 1) => (i.castSucc.succ, i.succ)) Finset.univ) ∪
          (Finset.image (fun i : Fin (n + 1) => (i.castSucc.succ, i.castSucc)) Finset.univ)) ∪
        ({(0, 0), (Fin.last (n + 2), Fin.last (n + 1))} :
          Finset (Fin (n + 2 + 1) × Fin (n + 1 + 1)))) := by
...
private lemma comm_succ (n : ℕ) :
  ...
  have h₁ : Disjoint (Finset.image γ₁ Sᶜ) (Finset.image γ₂ S) := disj1 n
  have h₂ : Disjoint (Finset.image γ₃ .univ) (Finset.image γ₄ .univ) := disj2 n
  have h₃ : Disjoint (Finset.disjUnion _ _ h₂) {(0, 0), (Fin.last _, Fin.last _)} := by
    simpa only [Finset.disjUnion_eq_union] using disj3 n
  have h₄ : Disjoint (Finset.disjUnion _ _ h₁) (Finset.disjUnion _ _ h₃) := by
    simpa only [Finset.disjUnion_eq_union] using disj4 n
  have H : (Finset.disjUnion _ _ h₁)ᶜ = Finset.disjUnion _ _ h₃ := by
    rw [compl_eq_iff_isCompl]
    refine ⟨h₄, codisjoint_iff.mpr (Finset.eq_univ_of_card _ ?_)⟩
    rw [Finset.sup_eq_union, Finset.card_union_of_disjoint h₄, Finset.card_disjUnion, Finset.card_disjUnion,
      Finset.card_disjUnion, Finset.card_image_of_injective _ hγ₁,
      Finset.card_image_of_injective _ hγ₂, Finset.card_image_of_injective _ hγ₃,
      Finset.card_image_of_injective _ hγ₄]
    simp
    lia
