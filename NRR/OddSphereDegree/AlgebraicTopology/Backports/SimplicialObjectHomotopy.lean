...
Only the module-system syntax (`module`/`public import`/`@[expose] public section`)
and the `using!` simpa variant (replaced by `using`) were adapted; the
mathematical content is upstream verbatim.

This file defines the notion of a combinatorial simplicial homotopy between two
morphisms of simplicial objects.
...
/-- The identity (degenerate) homotopy from `f` to `f`. -/
@[simps]
